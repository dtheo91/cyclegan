...
class Feature7Model(BaseModel):
    ...
    def forward(self):
        ...
        self.rec_A = torchvision.transforms.Resize((512, 512))(self.rec_A)
        mask_rec_A = self.mask_extractor(self.rec_A)[0].detach().argmax(1).unsqueeze(0)
        mask_rec_A = torchvision.transforms.Resize((256, 256))(mask_rec_A)
        self.rec_A = torchvision.transforms.Resize((256, 256))(self.rec_A)
        self.rec_mask_A= torch.cat([mask_rec_A, self.rec_A], dim=1)
        ...
        mask_rec_B = self.mask_extractor2(self.rec_B)[0].detach().argmax(1).unsqueeze(0)
        self.rec_mask_B = torch.cat([mask_rec_B, self.rec_B], dim=1)
    ...
    def backward_G(self):
        ...
        if lambda_idt > 0:
            ...
            mask_idt_A = self.mask_extractor2(self.idt_A)[0].detach().argmax(1).unsqueeze(0)
            self.idt_mask_A = torch.cat([mask_idt_A, self.idt_A], dim=1)
            self.loss_idt_A = self.criterionIdt(self.idt_mask_A, self.real_mask_B) * lambda_B * lambda_idt
            # G_B should be identity if real_A is fed: ||G_B(A) - A||
            self.idt_B = self.netG_B(self.real_mask_A)
            self.idt_B = torchvision.transforms.Resize((512, 512))(self.idt_B)
            mask_idt_B = self.mask_extractor(self.idt_B)[0].detach().argmax(1).unsqueeze(0)
            mask_idt_B = torchvision.transforms.Resize((256, 256))(mask_idt_B)
            self.idt_B = torchvision.transforms.Resize((256, 256))(self.idt_B)
            self.idt_mask_B = torch.cat([mask_idt_B, self.idt_B], dim=1)
            self.loss_idt_B = self.criterionIdt(self.idt_mask_B, self.real_mask_A) * lambda_A * lambda_idt
        else:
            self.loss_idt_A = 0
            self.loss_idt_B = 0
            
        # MSSIM LOSS 
        self.loss_mssim_A = (1 - self.msssim(self.rec_mask_A, self.real_mask_A)) * lambda_A
        self.loss_mssim_B = (1 - self.msssim(self.rec_mask_B, self.real_mask_B)) * lambda_B
        ...
        self.loss_cycle_A = self.criterionCycle(self.rec_mask_A, self.real_mask_A) * lambda_A
        # Backward cycle loss || G_A(G_B(B)) - B||
        self.loss_cycle_B = self.criterionCycle(self.rec_mask_B, self.real_mask_B) * lambda_B
        # combined loss and calculate gradients
